...
def dataset(f):
    ...
    from iccu.export import COL_MAPPER as col_mapper  # noqa: PLC0415
    from iccu.export import CSV_SCHEMA_OVERRIDES  # noqa: PLC0415

    csv = (
        pl.read_csv(f, schema_overrides=CSV_SCHEMA_OVERRIDES)
        .filter(~(pl.col("latitudine").is_null() | pl.col("longitudine").is_null()))
        .rename(col_mapper)
    )
